...
export default function ExamPage() {
  ...
  const { answers, answer, submitted, submit, timeLeft, timerActive, score, maxScore } =
    useExam(questions);
  ...
  const toggleOption = (idx: number) => {
    ...
    if (isMulti) {
      ...
      const updated = curr.includes(idx) ? curr.filter((x) => x !== idx) : [...curr, idx];
      answer(q.id, updated);
    } else {
      answer(q.id, idx);
      setTimeout(() => {
  ...
  const getOptionState = (idx: number): "idle" | "selected" | "correct" | "wrong" => {
        ...
        {showConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm">
            <div className="rounded-xl border border-blue-100 bg-white shadow-2xl p-6 max-w-sm w-full mx-4">
              ...
              <h3 className="font-bold text-slate-900 mb-1">Submit Exam?</h3>
